...
def find_audio_device():
    """Parses 'aplay -l' to find available audio devices dynamically."""
    print("[AUDIO] Scanning for audio hardware...")
    try:
        # Run aplay -l to list all hardware playback devices
        output = subprocess.check_output(["aplay", "-l"], stderr=subprocess.STDOUT, universal_newlines=True)
        devices = []
        
        for line in output.split('\n'):
            # Regex to match: "card 1: Audio [USB Audio], device 0: USB Audio [USB Audio]"
            match = re.search(r"card (\d+): (.*?) \[.*?\], device (\d+)", line)
            if match:
                card_num, card_name, dev_num = match.groups()
                dev_str = f"plughw:{card_num},{dev_num}"
                devices.append((dev_str, card_name.lower()))
        
        if not devices:
            print("[AUDIO WARNING] No hardware found. Falling back to 'default'.")
            return "default"
            
        # Prioritize USB/External Audio cards over onboard HDMI/Tegra audio
        for dev_str, name in devices:
            if "usb" in name or "audio" in name:
                print(f"[AUDIO] Auto-detected USB/External Audio: {dev_str}")
                return dev_str
                
        # Fallback to the first available device
        first_dev = devices[0][0]
        print(f"[AUDIO] Auto-detected Default Hardware: {first_dev}")
        return first_dev
        
    except Exception as e:
        print(f"[AUDIO ERROR] Detection failed: {e}. Falling back to 'default'.")
        return "default"
